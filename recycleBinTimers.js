...
export async function loadRecycleBinTimers(searchTerm = '') {
    ...
    try {
        ...
        sortedClients.forEach(clientName => {
            ...
            header.querySelector('.rbt-restore-client-btn').addEventListener('click', async (e) => {
                e.stopPropagation();
                const confirmed = await notify.confirm('Ripristinare tutti?', `Ripristinare tutti i timer di "${clientName}"?`, { icon: 'question', confirmText: 'Sì, ripristina!', confirmColor: '#10b981' });
                if (confirmed) restoreClientTimers(clientName);
            });

            // Elimina definitivamente per cliente
            header.querySelector('.rbt-delete-client-btn').addEventListener('click', async (e) => {
                e.stopPropagation();
                const confirmed = await notify.confirm('Eliminare definitivamente?', `Eliminare tutti i timer di "${clientName}"? Non potranno essere recuperati.`, { confirmText: 'Sì, elimina!' });
                if (confirmed) permanentlyDeleteClientTimers(clientName, section);
            });
...
export async function restoreTimer(timerId, rowElement) {
    try {
        ...
        rowElement.remove();
        notify.success('Ripristinato!', 'Timer ripristinato con successo.');
    } catch (error) {
...
export async function restoreClientTimers(clientName) {
    try {
        const snapshot = await db.collection('timeLogs')
            .where('uid', '==', currentUser.uid)
            .where('isDeleted', '==', true)
        ...
        await batch.commit();
        notify.success('Ripristinati!', `Tutti i timer di "${clientName}" ripristinati.`);
        loadRecycleBinTimers();
    } catch (error) {
...
export async function deleteTimer(timerId, barElement) {
    const confirmed = await notify.confirm('Sei sicuro?', 'Il timer sarà spostato nel cestino.', { confirmText: 'Sì, elimina!' });
    if (confirmed) {
        try {
            await db.collection('timeLogs').doc(timerId).update({
                isDeleted: true,
            ...
            notify.success('Timer Eliminato', 'Spostato nel cestino.');
        } catch (error) {
...
export async function undoDeleteTimer(timerId) {
    try {
        ...
        lastOperation = null;
        notify.success('Annullato', 'Il timer è stato ripristinato.');
...
export async function permanentlyDeleteTimer(timerId, rowElement) {
    const confirmed = await notify.confirm('Eliminare definitivamente?', 'Non potrà essere recuperato.', { confirmText: 'Sì, elimina!' });
    if (confirmed) {
        try {
            await db.collection('timeLogs').doc(timerId).delete();
            rowElement.remove();
            notify.success('Eliminato!', 'Timer eliminato definitivamente.');
        } catch (error) {
            console.error('Errore eliminazione:', error);
        }
    }
...
export async function permanentlyDeleteClientTimers(clientName, sectionElement) {
    try {
        const snapshot = await db.collection('timeLogs')
            .where('uid', '==', currentUser.uid)
            .where('isDeleted', '==', true)
        ...
        notify.success('Eliminati!', `Tutti i timer di "${clientName}" eliminati definitivamente.`);
    } catch (error) {
