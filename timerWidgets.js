...
// Cache in-memory: domain → { status: 'loading'|'loaded'|'failed', img?: HTMLImageElement, waiters: [] }
 ...
 * favicon from the domain. Uses in-memory cache to avoid repeated fetches.
 */
export function createFaviconEl(projectName, projectUrl, sizePx) {
    ...
    // Apply cached favicon or fetch
    if (domain) {
        const cached = faviconCache.get(domain);

        const applyFavicon = (srcImg) => {
            const clone = srcImg.cloneNode();
            clone.style.width = sizePx + 'px';
            clone.style.height = sizePx + 'px';
        ...
        if (cached && cached.status === 'loaded') {
            ...
            applyFavicon(cached.img);
        } else if (cached && cached.status === 'loading') {
            // Loading in progress — register waiter
            cached.waiters.push(applyFavicon);
        } else if (!cached || cached.status !== 'failed') {
            // First request — start loading
            const entry = { status: 'loading', img: null, waiters: [applyFavicon] };
            ...
            img.onload = () => {
                entry.status = 'loaded';
                entry.img = img;
                entry.waiters.forEach(fn => fn(img));
                entry.waiters = [];
            };
            img.onerror = () => {
                entry.status = 'failed';
