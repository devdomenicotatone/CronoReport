...
export async function loadRecycleBinReports(searchTerm = '') {
    ...
    try {
        ...
        sortedClients.forEach(clientName => {
            ...
            header.querySelector('.rb-restore-client-btn').addEventListener('click', async (e) => {
                e.stopPropagation();
                const confirmed = await notify.confirm('Ripristinare tutti?', `Ripristinare tutti i report di "${clientName}"?`, { icon: 'question', confirmText: 'Sì, ripristina tutti!', confirmColor: '#10b981' });
                if (confirmed) restoreClientReports(clientName);
            });

            header.querySelector('.rb-delete-client-btn').addEventListener('click', async (e) => {
                e.stopPropagation();
                const confirmed = await notify.confirm('Eliminare definitivamente?', `Eliminare definitivamente tutti i report di "${clientName}"? Non potranno essere recuperati.`, { confirmText: 'Sì, elimina!' });
                if (confirmed) permanentlyDeleteClientReports(clientName, section);
            });
...
export async function restoreReport(reportId, rowElement) {
    try {
        ...
        rowElement.remove();
        notify.success('Ripristinato!', 'Report ripristinato con successo.');
    } catch (error) {
...
export async function restoreClientReports(clientName) {
    try {
        ...
        await batch.commit();
        notify.success('Ripristinati!', `Tutti i report di "${clientName}" ripristinati.`);
        loadRecycleBinReports();
    } catch (error) {
...
export async function permanentlyDeleteReport(reportId, rowElement) {
    const confirmed = await notify.confirm('Eliminare definitivamente?', 'Questo report non potrà essere recuperato.', { confirmText: 'Sì, elimina!' });
    if (confirmed) {
        try {
            await db.collection('reports').doc(reportId).delete();
            rowElement.remove();
            notify.success('Eliminato!', 'Report eliminato definitivamente.');
        } catch (error) {
...
export async function permanentlyDeleteClientReports(clientName, sectionElement) {
    try {
        let query = db.collection('reports')
            .where('uid', '==', currentUser.uid)
            .where('isDeleted', '==', true);
        ...
        notify.success('Eliminati!', `Tutti i report di "${clientName}" eliminati definitivamente.`);
    } catch (error) {
